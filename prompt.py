...
**NOTE:- In case of a sentence for example: input.."greenhouse gases (GHG)" -> it should be converted to "GHG".**
🎯 TASK
Your task is to:
...
- Expanding acronyms on first use (e.g., United Nations (UN)).
...
• Expand acronyms more than once per section—if an acronym has already been expanded,
...
• Numbers:
- Spell out numbers 9 and below.
- Example: "5 projects" → "five projects."
- Use numerals for 10 and above.
- Example: "12 projects" stays as "12 projects."
- If a sentence starts with a number, spell it out or rewrite minimally to avoid starting with a
number.
- Example: "10 projects were funded" → "Ten projects were funded" or "The budget funded
10 projects."
• Fractions:
...
• Currency Format:
- Use the respective currency symbol (e.g., £, €, ¥, S$, US$).
- No space for smaller amounts; space for millions/billions.
- Example: "S$300" stays as "S$300," "US$2 million" stays as "US$2 million."
...
2. 📊 Percentages
• Use numerals + “per cent” in body text.
- Example: "5%" → "5 per cent," "5 percent" → "5 per cent."
...
- Convert numerical values with “°” or Fahrenheit/Celsius symbols to text.
- Example: "1.5°C" → "1.5 Degree Celsius", "98°F" → "98 Degree Fahrenheit".
- Applies only in body text.
...
• Acronyms: Expand only once per section.
- Example: After "United Nations (UN)," use "UN" in the same input.
...
1. 🇬🇧 Language & Spelling
   ...
   • For foreign words, wrap the term in single asterisks (term) to indicate italic formatting and provide the English translation in brackets on first use. For th terms that needs to be italic add "" in the start and end of that term for example: *term in the output.
