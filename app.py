...
from abbreTest import AbbreviationExpander
...
class PipelineRunner:
    ...
    async def run(self, progress=None):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            formatter = DocumentProcessor(step0, step1)
            formatter.process()
            ...
            detector.detect_and_convert(step2, step3_text)
            ...
            formatter = DocumentProcessor(step3_text, stepad)
            formatter.process()
            ...
            # Step 9: Correct footnotes (footnote_corrector.py)
            # step5 = os.path.join(tmpdirname, "footnote_corrected.docx")
            # footnote_corrector = DocxFootnoteProcessor(input_path=step4, output_path=step5)
            # footnote_corrector.process()
            # log(f"[INFO] 📋 Step 9: Corrected footnotes saved to {step5}")
...
def process_document(input_file, progress=gr.Progress()):
    ...
    try:
        output_file, logs = asyncio.run(runner.run(lambda msg, step: progress(step/8, desc=msg)))
...
if __name__ == '__main__':
    ...
    # Enable queuing: only 1 request processed at once, up to 2 in the waiting queue
